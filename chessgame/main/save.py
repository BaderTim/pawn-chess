...
class Save:
    ...
    def load_game(self):
        ...
        try:
            ...
            for counter, line in enumerate(file_lines):
                if counter == 0:
                    if ("m" or "ki") not in line:
                        print(f"\nFehler: Datei '{self.save_file}' scheint einen fehlerhaften Spielmodi zu haben.\n")
                        return None
                    save_load.append(line.replace("\n", ""))
                else:
                    data = line.replace(" ", "").replace("\n", "").split("_")
                    if data[0] != "w" and data[0] != "b":
                        print(f"\nFehler: Datei '{self.save_file}' scheint fehlerhafte Farben zu haben.\n")
                        return None
                    cords = data[1].split("#")
            ...
            print(f"\nFehler: Datei '{self.save_file}' scheint fehlerhaft zu sein.")
        return None
